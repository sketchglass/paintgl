...
class DrawTarget {
    ...
    */
    ...
    readPixels(rect, data, opts = {}) {
        this.use();
        const { gl } = this.context;
        rect = this._flipRect(rect);
        gl.readPixels(rect.left, rect.top, rect.width, rect.height, Texture_1.glFormat(gl, opts.format || this.pixelFormat), Texture_1.glType(this.context, opts.type || this.pixelType), data);
    }
    use() {
        const { gl } = this.context;
        if (this.scissor) {
